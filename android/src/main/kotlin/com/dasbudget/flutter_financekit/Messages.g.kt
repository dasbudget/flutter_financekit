...
data class ApiChanges (
  ...
  companion object {
    ...
    fun fromList(list: List<Any?>): ApiChanges {
      ...
      val inserted = list[1] as List<Any?>
      val newToken = ApiHistoryToken.fromList(list[2] as List<Any?>)
      val updated = list[3] as List<Any?>
...
interface FinanceKitApi {
  ...
  fun accountBalanceHistory(accountID: String): String
  fun transactionHistory(accountID: String): String
  fun accountHistory(): String
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: FinanceKitApi?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.flutter_financekit.FinanceKitApi.accountBalanceHistory", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            ...
            var wrapped: List<Any?>
            try {
              wrapped = listOf<Any?>(api.accountBalanceHistory(accountIDArg))
            } catch (exception: Throwable) {
              wrapped = wrapError(exception)
            }
            reply.reply(wrapped)
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.flutter_financekit.FinanceKitApi.transactionHistory", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val accountIDArg = args[0] as String
            var wrapped: List<Any?>
            try {
              wrapped = listOf<Any?>(api.transactionHistory(accountIDArg))
            } catch (exception: Throwable) {
              wrapped = wrapError(exception)
            }
            reply.reply(wrapped)
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.flutter_financekit.FinanceKitApi.accountHistory", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            var wrapped: List<Any?>
            try {
              wrapped = listOf<Any?>(api.accountHistory())
            } catch (exception: Throwable) {
              wrapped = wrapError(exception)
            }
            reply.reply(wrapped)
