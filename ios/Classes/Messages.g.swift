...
struct ApiChanges {
  ...
  static func fromList(_ list: [Any?]) -> ApiChanges? {
    ...
    let inserted = list[1] as! [Any?]
    let newToken = ApiHistoryToken.fromList(list[2] as! [Any?])!
    let updated = list[3] as! [Any?]
...
public class FinanceKitApiCodecReaderWriter: FlutterStandardReaderWriter {
...
class FinanceKitApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: FinanceKitApi?) {
    ...
    let accountBalanceHistoryChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.flutter_financekit.FinanceKitApi.accountBalanceHistory", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      accountBalanceHistoryChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let accountIDArg = args[0] as! String
        do {
          let result = try api.accountBalanceHistory(accountID: accountIDArg)
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
      ...
      accountBalanceHistoryChannel.setMessageHandler(nil)
    }
    let transactionHistoryChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.flutter_financekit.FinanceKitApi.transactionHistory", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      transactionHistoryChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let accountIDArg = args[0] as! String
        do {
          let result = try api.transactionHistory(accountID: accountIDArg)
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
      ...
      transactionHistoryChannel.setMessageHandler(nil)
    }
    let accountHistoryChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.flutter_financekit.FinanceKitApi.accountHistory", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      accountHistoryChannel.setMessageHandler { _, reply in
        do {
          let result = try api.accountHistory()
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
      ...
      accountHistoryChannel.setMessageHandler(nil)
    }
